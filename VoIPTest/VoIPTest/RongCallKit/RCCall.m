...
- (void)appDidBecomeActive {
    ...
    for ( UILocalNotification *notification in self.locationNotificationList ) {
        if ( [notification.userInfo[ @"appData" ][ @"callId" ]
                 isEqualToString:self.currentCallSession.callId] ) {
...
- (void)startSingleCallViewController:(NSString *)targetId
    ...
    //        initWithOutgoingCall:targetId mediaType:mediaType];
    //
...
- (void)startMultiCall:(RCConversationType)conversationType
    ...
    if ( [self preCheckForStartCall:mediaType] ) {
        [self checkSystemPermission:mediaType
                            success:^{
                                [self startSelectMemberViewContoller:
                                          conversationType
                                                            targetId:targetId
                                                           mediaType:mediaType];
                            }];
...
- (void)startSelectMemberViewContoller:(RCConversationType)conversationType
    ...
    //                           mediaType:mediaType
    //                               exist:@[[RCIMClient
    //                               sharedRCIMClient].currentUserInfo.userId]
    ...
    //        initWithRootViewController:voipCallSelectViewController];
...
- (void)startMultiCallViewController:(RCConversationType)conversationType
    ...
    if ( [self preCheckForStartCall:mediaType] ) {
        [self
            checkSystemPermission:mediaType
                          success:^{
                              //                                UIViewController*
                              //                                avCallVC = nil;
                              //                                if (mediaType ==
                              ...
                              //                                {
                              //                                    avCallVC =
                              //                                    [[RCCallAudioMultiCallViewController
                              //                                    alloc]
                              ...
                              //                                }
                              //                                [self
                              //                                presentCallViewController:avCallVC];
                          }];
    }
}
        ...
        [AVCaptureDevice
            requestAccessForMediaType:AVMediaTypeVideo
                    completionHandler:^( BOOL granted ) {
                        if ( !granted ) {
                            [self
                                loadErrorAlertWithConfirm:
                                    NSLocalizedStringFromTable(
                                        @"AccessRightTitle", @"RongCloudKit",
                                        nil )
                                                  message:
                                                      NSLocalizedStringFromTable(
                                                          @"cameraAccessRight",
...
- (BOOL)preCheckForStartCall:(RCCallMediaType)mediaType {
    RCCallSession *currentCallSession =
        [RCCall sharedRCCall].currentCallSession;
    if ( currentCallSession &&
         currentCallSession.mediaType == RCCallMediaAudio ) {
...
- (void)presentCallViewController:(UIViewController *)viewController {
    ...
    NSInteger checker =
        [RCCallKitUtility compareVersion:[UIDevice currentDevice].systemVersion
                               toVersion:@"13.0"];
    if ( checker >= 0 ) {
#ifdef __IPHONE_13_0
        [activityWindow setWindowScene:[UIApplication sharedApplication]
...
- (void)dismissCallViewController:(UIViewController *)viewController {
    ...
    //    [[UIApplication sharedApplication]
    //    setStatusBarStyle:UIStatusBarStyleDefault]; if ([viewController
    //    isKindOfClass:[RCCallBaseViewController class]]) {
    ...
    //            makeKeyWindow]; [self.callWindows removeObject:window]; break;
    //        }
    //    }
...
- (void)didReceiveCallRemoteNotification:(NSString *)callId
    ...
    [self didReceiveCallRemoteNotification:callId
    ...
    dispatch_async( dispatch_get_main_queue(), ^{
        UIAlertView *alert = [[UIAlertView alloc]
                initWithTitle:title
                      message:message
                     delegate:nil
            cancelButtonTitle:NSLocalizedStringFromTable( @"OK",
                                                          @"RongCloudKit", nil )
            otherButtonTitles:nil];
...
- (void)stopReceiveCallVibrate {
    ...
    if ( self.currentCallSession.callStatus == RCCallIncoming ||
         self.currentCallSession.callStatus == RCCallRinging ) {
